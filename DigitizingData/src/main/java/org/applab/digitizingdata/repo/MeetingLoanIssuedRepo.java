...
public class MeetingLoanIssuedRepo {
    ...
    public boolean saveMemberLoanIssue(int meetingId, int memberId, int loanNo, double amount) {
        SQLiteDatabase db = null;
        ...
        try {
            ...
            //Set Balance to be same as Principal Amount
            values.put(LoanIssueSchema.COL_LI_BALANCE, amount);
    ...
    public ArrayList<MemberLoanIssueRecord> getLoansIssuedToMemberInCycle(int cycleId, int memberId) {
        ...
        try {
            ...
            String query = String.format("SELECT  L.%s AS LoanId, M.%s AS MeetingDate, L.%s AS PrincipalAmount, " +
                    "L.%s AS LoanNo, L.%s AS Balance, L.%s AS IsCleared, L.%s AS DateCleared " +
    ...
    public MeetingLoanIssued getMostRecentLoanIssuedToMember(int memberId) {
        ...
        try {
            ...
            db = DatabaseHandler.getInstance(context).getWritableDatabase();
            String query = String.format("SELECT  L.%s AS LoanId, L.%s AS MeetingId, L.%s AS PrincipalAmount, " +
                    "L.%s AS LoanNo, L.%s AS Balance, L.%s AS TotalRepaid, L.%s AS IsCleared, L.%s AS DateCleared " +
                    " FROM %s AS L WHERE L.%s=%d AND (L.%s IS NULL OR L.%s = 0) ORDER BY L.%s DESC LIMIT 1",
                    LoanIssueSchema.COL_LI_LOAN_ID,LoanIssueSchema.COL_LI_MEETING_ID, LoanIssueSchema.COL_LI_PRINCIPAL_AMOUNT,
                    LoanIssueSchema.COL_LI_LOAN_NO, LoanIssueSchema.COL_LI_BALANCE, LoanIssueSchema.COL_LI_TOTAL_REPAID,
                    LoanIssueSchema.COL_LI_IS_CLEARED, LoanIssueSchema.COL_LI_DATE_CLEARED,
    ...
    public boolean updateMemberLoanBalances(int loanId, double totalRepaid, double balance) {
        SQLiteDatabase db = null;
        boolean performUpdate = false;
